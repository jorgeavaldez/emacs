...
* 🚨 NEOVIM -> EMACS SURVIVAL CHEATSHEET 🚨
** Key notation
- =C-= means Ctrl
...
- In Dired:
  ...
  - =g= refresh listing
  - =m= mark file, =u= unmark file, =U= unmark all
  - =C= copy marked/current files
  - =R= rename/move marked/current files
  - =+= create directory
  - =M-x dired-create-empty-file= create empty file
  - =C-x C-f= create/open file path in this directory
- 2-pane workflow (Oil-like): open 2 Dired windows, mark files in source, press =R= (move) or =C= (copy), then =RET= to accept the other Dired window as target
...
- Edit names/paths directly in buffer
  - Move across directories by editing to a relative/absolute path, e.g. =foo.txt= -> =../archive/foo.txt=
...
#+begin_src emacs-lisp
  ...
  ;; Keep repo folders clean:
  ;; - disable lockfiles (".#file")
  ;; - send autosaves ("#file#") and backups ("file~") to user-emacs-directory
  (setq create-lockfiles nil)

  (let ((autosave-dir (expand-file-name "auto-save/" user-emacs-directory))
...
we use three layers:
- https://github.com/purcell/exec-path-from-shell for startup shell env in gui emacs
- https://github.com/purcell/inheritenv so temp-buffer commands inherit buffer env
- https://github.com/eki3z/mise.el for per-project env (mise.toml/.venv) per buffer
...
#+begin_src emacs-lisp
  ...
  ;; mise.el depends on inheritenv and uses it for process propagation.
  ...
  (use-package mise
    ...
    :config
    ;; Keep default trust behavior ('ask') for safety.
    ;; Use M-x mise-reload if you need to force a refresh for current buffer.
...
* Dired + Wdired (2-pane workflow)
This keeps Dired fast for cross-directory file moves/copies.
...
#+begin_src emacs-lisp
  (use-package dired
    ...
    :custom
    ;; If two Dired windows are open, default copy/rename target to the other one.
    ;; This is the closest built-in flow to Oil-style "operate in one pane, send to another".
    (dired-dwim-target t)
    ;; Avoid repeated prompts when operating on directories.
...
#+begin_src emacs-lisp
  ...
  (defvar jorge/eglot-ts-contact-candidates
    ;; Prefer mise-managed tooling; bunx will use local package
    ;; binaries when present and otherwise fetch a cached copy.
    '(("mise" "x" "--" "bunx" "--bun" "typescript-language-server" "--stdio")
      ("typescript-language-server" "--stdio")
      ("bunx" "--bun" "typescript-language-server" "--stdio")
      ("npx" "typescript-language-server" "--stdio")))

  (defun jorge/eglot-tsserver-path (&optional dir)
    "Return nearest node_modules tsserver.js path from DIR upward."
    (let* ((start (or dir default-directory))
           (root (locate-dominating-file start "node_modules/typescript/lib/tsserver.js")))
      (when root
        (expand-file-name "node_modules/typescript/lib/tsserver.js" root))))

  (defun jorge/eglot-ts-contact (interactive _project)
    "Build TypeScript LSP contact with monorepo-aware tsserver path.

The project root may be a top-level VCS directory in monorepos.  Passing
`tsserver.path' from the nearest node_modules avoids startup failures."
    (let* ((contact (funcall (eglot-alternatives jorge/eglot-ts-contact-candidates)
                             interactive _project))
           (tsserver-path (jorge/eglot-tsserver-path)))
      (if (and (listp contact) tsserver-path)
          (append contact
  ...
  (use-package eglot
    ...
    (let ((py-server (eglot-alternatives
                      ;; Prefer running python LSP servers through mise so
                      ;; project-local env/venv from mise.toml is applied.
                      '(("mise" "x" "--" "basedpyright-langserver" "--stdio")
                        ("mise" "x" "--" "pyright-langserver" "--stdio")
                        ("basedpyright-langserver" "--stdio")
                        ("pyright-langserver" "--stdio"))))
          (ts-server #'jorge/eglot-ts-contact))
    ...
    ;; Keep VCS-root project behavior by default.
    ;; JS/TS uses `jorge/eglot-ts-contact' to pass `tsserver.path' from the
    ;; nearest node_modules so monorepos still initialize correctly.
  ...
  ;; Floating docs near point (inline popup) for Eglot/ElDoc.
  ;; Keep this on-demand to avoid constant hover noise while editing.
  (use-package eldoc-box
    :after eglot
    :commands (eldoc-box-help-at-point)
    :bind (:map jorge/leader-map
  ...
  (defun jorge/treesit--motion-predicate (kind)
    ...
    (let ((lang (treesit-language-at (point))))
      (cond
       ...
       ;; Emacs 30 expects (treesit-thing-defined-p THING LANGUAGE).
       ((and (eq kind 'function)
             lang
             (treesit-thing-defined-p 'defun lang))
        'defun)
       (t
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'evil
    (with-eval-after-load 'majutsu
      ;; In Evil normal state, bare `G` is `evil-goto-line` (end of buffer).
      ;; Rebind it in Majutsu buffers to the Majutsu Git transient.
...
#+end_src
** diff viewing
ediff is the default diff editor/viewer.
by default it pops up a weird little window and we have to be in that window to send commands.
we can change that though.
#+begin_src emacs-lisp
  ;; Use a single frame for Ediff instead of a popup control window
  (setq ediff-window-setup-function 'ediff-setup-windows-plain)
  ;; Split windows horizontally (side-by-side) instead of vertically
...
#+begin_src emacs-lisp
    (use-package pi-coding-agent
      ...
      ;; Use my GitHub branch directly via package-vc (instead of local checkout).
      :vc (:url "https://github.com/jorgeavaldez/pi-coding-agent.git"
           :branch "dev"
           :rev :newest)
    ...
    (defun jorge/pi--chat-buffer-p (buffer)
      "Return non-nil if BUFFER is a live pi chat buffer."
      (and (buffer-live-p buffer)
           (with-current-buffer buffer
             (derived-mode-p 'pi-coding-agent-chat-mode))))

    (defun jorge/pi--session-buffer-p (buffer)
      "Return non-nil if BUFFER is a live pi chat/input buffer."
      (and (buffer-live-p buffer)
           (with-current-buffer buffer
             (derived-mode-p 'pi-coding-agent-chat-mode 'pi-coding-agent-input-mode))))

    (defun jorge/pi--session-input-buffer (chat-buf)
      "Return input buffer paired with CHAT-BUF, or nil."
      (when (jorge/pi--chat-buffer-p chat-buf)
        (let ((input-buf (buffer-local-value 'pi-coding-agent--input-buffer chat-buf)))
          (when (buffer-live-p input-buf)
            input-buf))))

    (defun jorge/pi--session-buffers (&optional buffer)
      "Return (CHAT INPUT) for BUFFER's session, or nil if BUFFER is not a pi buffer."
      (let ((buf (or buffer (current-buffer))))
        (when (jorge/pi--session-buffer-p buf)
          (with-current-buffer buf
            (let* ((chat (if (derived-mode-p 'pi-coding-agent-chat-mode)
                             (current-buffer)
                           pi-coding-agent--chat-buffer))
                   (input (if (derived-mode-p 'pi-coding-agent-input-mode)
                              (current-buffer)
                            pi-coding-agent--input-buffer)))
              (when (and (buffer-live-p chat) (buffer-live-p input))
                (list chat input)))))))

    (defun jorge/pi--live-session-count ()
      "Return count of live pi chat session buffers."
      (length (jorge/pi--live-chat-sessions)))

    (defun jorge/pi--live-chat-sessions ()
      "Return all live pi chat session buffers in buffer-list order."
      (let (sessions)
        (dolist (buf (buffer-list))
          (when (jorge/pi--chat-buffer-p buf)
            (push buf sessions)))
        (nreverse sessions)))

    (defun jorge/pi--visible-chat-sessions ()
      "Return unique live pi chat buffers currently visible in this frame."
      (let (sessions)
        (dolist (win (window-list nil 'no-minibuffer))
          (let* ((buf (window-buffer win))
                 (chat (cond
                        ((jorge/pi--chat-buffer-p buf) buf)
                        ((jorge/pi--session-buffer-p buf)
                         (with-current-buffer buf
                           pi-coding-agent--chat-buffer)))))
            (when (and (buffer-live-p chat)
                       (not (memq chat sessions)))
              (push chat sessions))))
        (nreverse sessions)))

    (defun jorge/pi--selected-non-pi-buffer ()
      "Return selected window's buffer when it is non-pi, else nil."
      (let ((buf (window-buffer (selected-window))))
        (when (and (buffer-live-p buf)
                   (not (jorge/pi--session-buffer-p buf)))
          buf)))

    (defun jorge/pi--first-visible-non-pi-buffer ()
      "Return first visible non-pi buffer in current frame, or nil."
      (let (result)
        (dolist (win (window-list nil 'no-minibuffer))
          (let ((buf (window-buffer win)))
            (when (and (not result)
    ...
    (defun jorge/pi--display-session-default (chat-buf input-buf target-window)
      "Display CHAT-BUF and INPUT-BUF in TARGET-WINDOW with pi's default layout."
      (select-window target-window)
      ;; Reuse pi's own display helper so input height stays package-default.
      (pi-coding-agent--display-buffers chat-buf input-buf)
      ;; Keep focus in input (same as normal sending flow).
      (let ((input-win (get-buffer-window input-buf)))
        (when (window-live-p input-win)
          (select-window input-win))))

    (defun jorge/pi--reset-session-fullframe (chat-buf input-buf)
      "Reset CHAT-BUF/INPUT-BUF to a fresh full-frame pi layout."
      (delete-other-windows)
      (jorge/pi--display-session-default chat-buf input-buf (selected-window)))

    (defun jorge/pi--focus-input (input-buf)
      "Focus INPUT-BUF if it is visible in a live window."
      (let ((win (get-buffer-window input-buf)))
        (when (window-live-p win)
          (select-window win))))

    (defun jorge/pi--first-visible-other-session (target-chat)
      "Return (CHAT INPUT) for first visible session that is not TARGET-CHAT."
      (let (found)
        (dolist (chat (jorge/pi--visible-chat-sessions))
          (let ((input (jorge/pi--session-input-buffer chat)))
            (when (and (not found)
                       (buffer-live-p input)
                       (not (eq chat target-chat)))
              (setq found (list chat input)))))
        found))

    (defun jorge/pi--layout-buffer-and-session (context-buffer chat-buf input-buf &optional session-in-first-window)
      "Create a 50/50 side split with CONTEXT-BUFFER and pi CHAT-BUF/INPUT-BUF.
  When SESSION-IN-FIRST-WINDOW is non-nil, keep session in the selected
  window's half and show CONTEXT-BUFFER in the newly split half."
      (delete-other-windows)
      (let ((first-win (selected-window))
            (second-win (split-window-right)))
        (if session-in-first-window
            (progn
              (jorge/pi--display-session-default chat-buf input-buf first-win)
              (set-window-buffer second-win context-buffer))
          (set-window-buffer first-win context-buffer)
          (jorge/pi--display-session-default chat-buf input-buf second-win))))

    (defun jorge/pi--layout-two-sessions (chat-a input-a chat-b input-b &optional session-a-in-first-window)
      "Create a 50/50 side split with two pi sessions.
  When SESSION-A-IN-FIRST-WINDOW is non-nil, keep CHAT-A/INPUT-A in the
  selected window's half and show CHAT-B/INPUT-B in the new half."
      (delete-other-windows)
      (let ((first-win (selected-window))
            (second-win (split-window-right)))
        (if session-a-in-first-window
            (progn
              (jorge/pi--display-session-default chat-a input-a first-win)
              (jorge/pi--display-session-default chat-b input-b second-win))
          (jorge/pi--display-session-default chat-b input-b first-win)
          (jorge/pi--display-session-default chat-a input-a second-win))))

    (defun jorge/pi--session-candidate (chat-buf)
      "Return completion candidate pair (DISPLAY . CHAT-BUF) for CHAT-BUF."
      (let ((buf-name (buffer-name chat-buf))
            session-name)
        (with-current-buffer chat-buf
          (setq session-name (and (boundp 'pi-coding-agent--session-name)
                                  pi-coding-agent--session-name)))
        (cons (if (and session-name (not (string-empty-p session-name)))
                  (format "%s  [%s]" session-name buf-name)
                buf-name)
              chat-buf)))

    (defun jorge/pi ()
      "Start a named pi session in a user-selected directory.
  Defaults session name to \"session-N\" based on live pi sessions.
  Prompts for directory, defaulting to project root via `jorge/default-dir',
  falling back to `default-directory'."
      (interactive)
      (require 'pi-coding-agent)
      (let* ((default-name (format "session-%d" (1+ (jorge/pi--live-session-count))))
             (session-name (read-string "Pi session name: " default-name nil default-name))
             (dir (file-name-as-directory
                   (expand-file-name
                    (read-directory-name "Pi directory: " (jorge/default-dir) nil t))))
             chat-buf
             input-buf)
        (pi-coding-agent--check-dependencies)
        (setq chat-buf (pi-coding-agent--setup-session dir session-name))
        (setq input-buf (buffer-local-value 'pi-coding-agent--input-buffer chat-buf))
        (pi-coding-agent--display-buffers chat-buf input-buf)))

    (defun jorge/pi-summon-paired-buffer ()
      "Summon/reset current pi pair.
  Only meaningful from pi chat/input buffers.
  If a non-pi buffer is visible, split 50/50 and keep the current pi
  session in one half while putting that non-pi buffer in the other.
  Otherwise reset pi to fresh full-frame layout."
      (interactive)
      (require 'pi-coding-agent)
      (let* ((session (jorge/pi--session-buffers))
             (chat-buf (car session))
             (input-buf (cadr session)))
        (unless (and (buffer-live-p chat-buf) (buffer-live-p input-buf))
          (user-error "SPC a b is only available in pi chat/input buffers"))
        (let ((context-buf (or (jorge/pi--selected-non-pi-buffer)
                               (jorge/pi--first-visible-non-pi-buffer))))
          (if (buffer-live-p context-buf)
              (progn
                (jorge/pi--layout-buffer-and-session context-buf chat-buf input-buf t)
                (jorge/pi--focus-input input-buf))
            (jorge/pi--reset-session-fullframe chat-buf input-buf)))))

    (defun jorge/pi-switch-live-session ()
      "Switch between live Emacs pi sessions (chat buffers only).
  Uses existing minibuffer stack (vertico/orderless/marginalia).
  Selecting current session from a pi buffer performs a full-frame reset;
  other selections build a 50/50 side layout with sensible context."
      (interactive)
      (require 'pi-coding-agent)
      (let ((sessions (jorge/pi--live-chat-sessions)))
        (if (null sessions)
            (user-error "No live pi sessions")
          (let* ((choices (mapcar #'jorge/pi--session-candidate sessions))
                 (selection (completing-read "Pi live session: "
                                             (mapcar #'car choices)
                                             nil t))
                 (target-chat (cdr (assoc selection choices)))
                 (target-input (and (buffer-live-p target-chat)
                                    (jorge/pi--session-input-buffer target-chat)))
                 (current-session (jorge/pi--session-buffers))
                 (current-chat (car current-session))
                 (current-input (and (buffer-live-p current-chat)
                                     (jorge/pi--session-input-buffer current-chat)))
                 (visible-other (jorge/pi--first-visible-other-session target-chat))
                 (context-buf (or (jorge/pi--selected-non-pi-buffer)
                                  (jorge/pi--first-visible-non-pi-buffer))))
            (unless (and (buffer-live-p target-chat) (buffer-live-p target-input))
              (user-error "Selected pi session is no longer live"))
            (cond
             ((and (buffer-live-p current-chat)
                   (eq current-chat target-chat))
              (jorge/pi--reset-session-fullframe target-chat target-input))
             ((and (buffer-live-p current-chat)
                   (buffer-live-p current-input))
              (jorge/pi--layout-two-sessions
               current-chat current-input
               target-chat target-input
               t)
              (jorge/pi--focus-input target-input))
             ((and (consp visible-other)
                   (buffer-live-p (car visible-other))
                   (buffer-live-p (cadr visible-other)))
              (jorge/pi--layout-two-sessions
               (car visible-other) (cadr visible-other)
               target-chat target-input
               t)
              (jorge/pi--focus-input target-input))
             ((buffer-live-p context-buf)
              (jorge/pi--layout-buffer-and-session context-buf target-chat target-input nil)
              (jorge/pi--focus-input target-input))
             (t
              (jorge/pi--reset-session-fullframe target-chat target-input)))))))

    (define-key jorge/leader-map (kbd "a i") #'jorge/pi)
    (define-key jorge/leader-map (kbd "a b") #'jorge/pi-summon-paired-buffer)
    (define-key jorge/leader-map (kbd "a s") #'jorge/pi-switch-live-session)

    (with-eval-after-load 'which-key
      (which-key-add-key-based-replacements
        "SPC a" "ai"
        "SPC a i" "start named pi session"
        "SPC a b" "summon/reset pi pair"
        "SPC a s" "switch live pi session"))
#+end_src

* Notes / Obsidian (obsidian.el)
keep this intentionally small and binding-first.
mirror your old neovim flow where it matters most.
#+begin_src emacs-lisp
  (defconst jorge/obsidian-vault-dir (expand-file-name "~/obsidian/delvaze")
    "Absolute path to your Obsidian vault.")

  (defun jorge/obsidian-in-vault-p (&optional file)
    "Return non-nil when FILE (or current buffer file) lives in the Obsidian vault."
    (let* ((target (or file (buffer-file-name)))
           (vault (file-name-as-directory (expand-file-name jorge/obsidian-vault-dir))))
      (and target
           (string-prefix-p vault (expand-file-name target)))))

  (defun jorge/obsidian-consult-ripgrep-vault ()
    "Search Obsidian vault with Consult ripgrep."
    (interactive)
    (consult-ripgrep jorge/obsidian-vault-dir))

  (defun jorge/obsidian-new-prompt-note ()
    "Create/open a prompt note under <vault>/prompts."
    (interactive)
    (require 'subr-x)
    (let* ((title (string-trim (read-string "Prompt note title: ")))
           (prompts-dir (expand-file-name "prompts" jorge/obsidian-vault-dir)))
      (when (string-empty-p title)
        (user-error "Prompt note title cannot be empty"))
      (make-directory prompts-dir t)
      (find-file (expand-file-name (concat title ".md") prompts-dir))))

  (defun jorge/obsidian-template-title ()
    "Return a reasonable template title for the current buffer context."
    (let ((raw (if (buffer-file-name)
                   (file-name-base (buffer-file-name))
                 (buffer-name))))
      ;; Scratch/special buffers often use *name*; strip wrappers for cleaner titles.
      (string-trim (replace-regexp-in-string "\\`\\*\\|\\*\\'" "" raw))))

  (defun jorge/obsidian-apply-template ()
    "Insert a template from <vault>/templates at point in the current buffer.

Works in any writable buffer (file-backed or not)."
    (interactive)
    (require 'subr-x)
    (let* ((templates-dir (expand-file-name "templates" jorge/obsidian-vault-dir))
           (files (and (file-directory-p templates-dir)
                       (directory-files-recursively templates-dir "\\.md\\'"))))
      (unless files
        (user-error "No templates found in %s" templates-dir))
      (let* ((choices (mapcar (lambda (f) (file-relative-name f templates-dir)) files))
             (choice (completing-read "Template: " choices nil t))
             (template-file (expand-file-name choice templates-dir))
             (template-content (with-temp-buffer
                                 (insert-file-contents template-file)
                                 (buffer-string)))
             (rendered (let ((title (jorge/obsidian-template-title))
                             (date (format-time-string "%Y-%m-%d"))
                             (time (format-time-string "%H:%M:%S")))
                         (setq template-content
                               (replace-regexp-in-string "{{title}}" title template-content))
                         (setq template-content
                               (replace-regexp-in-string "{{date}}" date template-content))
                         (replace-regexp-in-string "{{time}}" time template-content))))
        (insert rendered)
        (message "Inserted template: %s" choice))))

  (defun jorge/obsidian-follow-link-at-point-vault-only ()
    "Follow Obsidian link at point only for files inside the vault."
    (interactive)
    (if (jorge/obsidian-in-vault-p)
        (progn
          (require 'obsidian)
          (obsidian-follow-link-at-point))
      (message "Not in Obsidian vault; no follow-link action")))

  ;; Vendored locally from https://github.com/knu/obsidian-pcache.el
  ;; (not installed as a package) so we can review/track the exact source.
  (use-package pcache
    :ensure t)
  ...
  (use-package obsidian-pcache
    :ensure nil
    :demand t
    :load-path (lambda () (list (expand-file-name "lisp" user-emacs-directory))))

  (use-package obsidian
    :demand t
    :init
    ;; matching your high-value old mappings only.
    (define-key jorge/leader-map (kbd "o n") #'obsidian-capture)
    (define-key jorge/leader-map (kbd "o d") #'obsidian-daily-note)
    (define-key jorge/leader-map (kbd "o f") #'obsidian-jump)
    (define-key jorge/leader-map (kbd "o /") #'jorge/obsidian-consult-ripgrep-vault)
    (define-key jorge/leader-map (kbd "o p") #'jorge/obsidian-new-prompt-note)
    (define-key jorge/leader-map (kbd "o t") #'jorge/obsidian-apply-template)

    ;; keep SPC RET predictable: only obsidian behavior in vault files.
    (define-key jorge/leader-map (kbd "RET") #'jorge/obsidian-follow-link-at-point-vault-only)
    :custom
    (obsidian-directory jorge/obsidian-vault-dir)
    (obsidian-inbox-directory "inbox")
    (obsidian-daily-notes-directory "daily")
    (obsidian-templates-directory "templates")
    ;; keep wiki links recognized for follow-link behavior.
  ...
  (with-eval-after-load 'which-key
    (which-key-add-key-based-replacements
      "SPC o" "obsidian"
      "SPC o n" "new note (inbox)"
      "SPC o d" "daily note"
      "SPC o f" "quick switch note"
      "SPC o /" "search vault (ripgrep)"
      "SPC o p" "new prompt note"
      "SPC o t" "insert template at point"
      "SPC RET" "follow obsidian link (vault only)"))
#+end_src
...
* next up!
** TODO split the pi input window correctly
when we split vertical, the chat input window stays 100vw which is nasty. i want it to split along with the chat buffer.
** TODO investigate winner-mode for layout undo/redo
=winner-mode= is a built-in Emacs feature that tracks window layout history.
think of it like /undo and redo, but for split arrangements instead of text edits.

what it is:
- built into Emacs (no external package needed)
- keeps a history stack of window configurations (splits, buffer placement, sizes)
- lets me quickly roll back a bad layout change

what it does for my workflow:
- when pi or other commands reorganize windows, i can pop back to the previous layout instantly
- when i "overshoot" while experimenting with split changes, i can step backward/forward through layouts
- reduces friction from constant file/buffer/session switching because layout mistakes become cheap

likely commands to use:
- =winner-undo= :: restore the previous window layout
- =winner-redo= :: go forward again after undoing

ticket goal:
- evaluate if winner-mode smooths out the pi split/reset workflow enough to make it default
- if yes, add minimal config + leader bindings in a later change
- if no, document why and keep current custom layout helpers only

* wat
#+begin_src emacs-lisp
   (advice-add
    'pi-coding-agent--start-process :before
    (lambda (dir)
      (message "[pi-start] %s dir=%s cmd=%S"
               (format-time-string "%H:%M:%S.%3N")
               dir
               (list "pi" "--mode" "rpc" pi-coding-agent-extra-args))))
#+end_src
