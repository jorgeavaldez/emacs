...
#+begin_src emacs-lisp
    (use-package pi-coding-agent
      ...
      :vc (:url "https://github.com/jorgeavaldez/pi-coding-agent.git"
           :branch "feat/rpc/support-editor-resolve-session-changes"
           :rev :newest)
...
#+begin_src emacs-lisp
  ...
  (defun jorge/obsidian-apply-template ()
    "Apply a template from <vault>/templates to current note."
    (interactive)
    (require 'obsidian)
    (unless (buffer-file-name)
      (user-error "Current buffer is not visiting a file"))
    (unless (jorge/obsidian-in-vault-p)
      (user-error "Current file is not inside the Obsidian vault"))
    (let* ((templates-dir (expand-file-name "templates" jorge/obsidian-vault-dir))
      ...
      (let* ((choices (mapcar (lambda (f) (file-relative-name f templates-dir)) files))
        ...
        (when (and (> (buffer-size) 0)
                   (not (y-or-n-p "Current note is not empty. Insert template anyway? ")))
          (user-error "Template insertion cancelled"))
        (obsidian-apply-template template-file))))
