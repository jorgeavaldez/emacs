...
#+begin_src emacs-lisp
  ...
  (defun jorge/pi--layout-non-pi-left-session-right (left-buffer right-chat right-input)
    "Create 50/50 split: LEFT-BUFFER on left, RIGHT-CHAT/RIGHT-INPUT on right."
    (delete-other-windows)
    (let ((left-win (selected-window))
          right-win)
      (set-window-buffer left-win left-buffer)
      (setq right-win (split-window-right))
      (balance-windows)
      ;; New split content always lives on the right.
      (jorge/pi--display-session-default right-chat right-input right-win)
      (balance-windows)))

  (defun jorge/pi--layout-session-left-session-right (left-chat left-input right-chat right-input)
    "Create 50/50 split with pi session LEFT and pi session RIGHT."
    (delete-other-windows)
    (let ((left-win (selected-window))
          right-win)
      (setq right-win (split-window-right))
      (balance-windows)
      (jorge/pi--display-session-default left-chat left-input left-win)
      ;; New split content always lives on the right.
      (jorge/pi--display-session-default right-chat right-input right-win)
      (balance-windows)))

  (defun jorge/pi--choose-left-context (target-chat &optional current-chat)
    "Pick left-half context while showing TARGET-CHAT on right.
Prefer CURRENT-CHAT when provided and distinct from TARGET-CHAT,
then any visible other pi session, then selected non-pi buffer."
    (let ((current-input (and (buffer-live-p current-chat)
                              (jorge/pi--session-input-buffer current-chat))))
      (cond
       ((and (buffer-live-p current-chat)
             (buffer-live-p current-input)
             (not (eq current-chat target-chat)))
        (list :session current-chat current-input))
       (t
        (let ((visible (jorge/pi--visible-chat-sessions))
              found)
          (dolist (chat visible)
            (let ((input (jorge/pi--session-input-buffer chat)))
              (when (and (not found)
                ...
                (setq found (list :session chat input)))))
          (or found
              (let ((buf (or (jorge/pi--selected-non-pi-buffer)
                             (jorge/pi--first-visible-non-pi-buffer))))
                (when (buffer-live-p buf)
                  (list :buffer buf)))))))))

  (defun jorge/pi--show-session-on-right (target-chat target-input &optional current-chat)
    "Show TARGET-CHAT/TARGET-INPUT on right, preserving best left context."
    (let ((left (jorge/pi--choose-left-context target-chat current-chat)))
      (cond
       ((and left (eq (car left) :session))
        (jorge/pi--layout-session-left-session-right
         (nth 1 left) (nth 2 left) target-chat target-input))
       ((and left (eq (car left) :buffer))
        (jorge/pi--layout-non-pi-left-session-right
         (nth 1 left) target-chat target-input))
       (t
...
If any non-pi buffer is visible, keep non-pi on left and open pi on right.
Otherwise reset pi to fresh full-frame layout."
    ...
    (let* ((session (jorge/pi--session-buffers))
      ...
      (let ((left-buf (jorge/pi--first-visible-non-pi-buffer)))
        (if (buffer-live-p left-buf)
            (jorge/pi--layout-non-pi-left-session-right left-buf chat-buf input-buf)
...
all other selections open selected session on right in a 50/50 split."
    (interactive)
    (require 'pi-coding-agent)
    (let ((sessions (jorge/pi--live-chat-sessions)))
      (if (null sessions)
        ...
        (let* ((choices (mapcar #'jorge/pi--session-candidate sessions))
          ...
          (if (and (buffer-live-p current-chat)
                   (eq current-chat target-chat))
              (jorge/pi--reset-session-fullframe target-chat target-input)
            (jorge/pi--show-session-on-right target-chat target-input current-chat))))))
